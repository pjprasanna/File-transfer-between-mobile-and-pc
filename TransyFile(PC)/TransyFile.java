...
public class TransyFile extends javax.swing.JFrame {
    ...
    static class SendFile extends Thread {
        ...
        public SendFile(String path, String fileName, String mobIP, JButton send) {
            this.path = path;
    ...
    static class ReceiveFile extends Thread {
        ...
        public ReceiveFile(JButton receive) throws IOException {
            this.receive = receive;

            ServerSoc = new ServerSocket(1908);

            if (System.getProperty("os.name").equals("Linux")) {
                fetchIP = new ProcessBuilder("/bin/sh", "-c",
                        "ip addr | grep \"inet \" | grep -v 127 | cut -d\" \" -f6 | cut -d\"/\" -f1").start();
        ...
        public void run() {
            while (exit) {
                ...
                try {
                    ...
                    din = new DataInputStream(ClientSoc.getInputStream());
                    filepath = new StringBuilder(System.getProperty("user.home") + "/Downloads/" + file);
